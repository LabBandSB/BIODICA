...
public class UseTOPPgeneSuiteForEnrichment {
	...
	public static void completeAnalysisOfMetaGeneFile(String sfile, String analysisName, String outPutFolder, float threshold, int minNumberGenes, boolean useInternet) throws Exception{
		...
		for(int i=0;i<vt.colCount;i++)if(vt.fieldTypes[i]==vt.NUMERICAL){
			...
			if(listpositive.size()>=minNumberGenes){
				fw.write("<td width=10%><a href=\""+colName+"plus.html\">"+colName+"+ </a>(<a href=\""+colName+"plus_genes.html\">"+listpositive.size()+" genes</a>)</td>\n");
				if(useInternet)
				...
				String annot = produceAnnotationFromHTMLfile(outPutFolder+colName+"plus.html");
				if(annot!=null) BestAnnotation = annot;
				fw.write("<td width=30%>"+BestAnnotation+"</td>\n");
				
			}else{
				fw.write("<td width=10%>"+colName+"+ (<a href=\""+colName+"plus_genes.html\">"+listpositive.size()+" genes</a>)</td>\n");
				fw.write("<td width=30%>"+BestAnnotation+"</td>\n");
			}}
			FileWriter fwlist = new FileWriter(outPutFolder+colName+"plus_genes.html");
			...
			if(listnegative.size()>=minNumberGenes){
				fw.write("<td width=10%><a href=\""+colName+"minus.html\">"+colName+"- </a>(<a href=\""+colName+"minus_genes.html\">"+listnegative.size()+" genes</a>)</td>\n");
				if(useInternet)
				...
				String annot = produceAnnotationFromHTMLfile(outPutFolder+colName+"minus.html");
				if(annot!=null) BestAnnotation = annot;
				fw.write("<td width=30%>"+BestAnnotation+"</td>\n");
			}else{
				fw.write("<td width=10%>"+colName+"- (<a href=\""+colName+"minus_genes.html\">"+listnegative.size()+" genes</a>)</td>\n");
				fw.write("<td width=30%>"+BestAnnotation+"</td>\n");
			}}
			fwlist = new FileWriter(outPutFolder+colName+"minus_genes.html");
	...
	/*
	 * Values are returned as 0-name, 1-link, 2-qval, 3-number of genes, 4-size of the categ 
	 */
