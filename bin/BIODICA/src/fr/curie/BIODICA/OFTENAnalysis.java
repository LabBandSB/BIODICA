...
public class OFTENAnalysis {
	...
	public String completeOFTENAnalysisOfTable(String fn, String name, int nstart, int nend, int nstep) throws Exception{
	...
	public Graph intersectGraphs(Vector<Graph> graphs){
	    Graph mergedGraph = fr.curie.BIODICA.Utils.MergeNetworkAndFilter(graphs, thresholdIntersectionOfGraphs);
		return mergedGraph;
	}
	...
	public static void MakeNetworkIntersectionGraph(String folder, String network_files[], String prefixes[], float score_threshold, float inters_threshold, String outfilename) throws Exception{
		...
		for(int i=0;i<fileNames.size();i++)for(int j=i+1;j<fileNames.size();j++){
			...
			int inters = fr.curie.BIODICA.Utils.compareTwoSets(ni, nj);
			int union = (fr.curie.BIODICA.Utils.UnionOfVectors(ni,nj)).size();
