...
public class MetaGeneAnnotation{
	...
	public static void MakeGSEAPrerankedAnalysis(String rnkFile, String gmtFile, int numberOfPermutations) throws Exception{
		...
		//for(File f:list)if(f.getName().endsWith(".rnk")){
			...
			argsv.add("-out"); argsv.add(folder+"/results/"+analysisName);
	...
	public static void FilterGSEAResults(String folder, int minNumberOfGenes, float scoreThreshold, float qFDRthreshold, float pFWERthreshold) throws Exception{
		float f[] = {1f,2f};
		...
		fw.write("<title>ICA analysis</title>\n");
